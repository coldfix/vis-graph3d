...
function setSurfaceColor(surfaceColors, dst) {
  ...
  if (Array.isArray(surfaceColors)) {
    ...
    surfaceColors.reverse();
    colormap = Color.interpolate(surfaceColors);
  } else if (typeof surfaceColors === 'object') {
    // for some reason surfaceColors goes from vMax to vMin:
    [surfaceColors.start, surfaceColors.end] =
      [surfaceColors.end, surfaceColors.start];
    colormap = Color.interpolate(parseColorObject(surfaceColors.hue));
  } else {
...
function setColormap(colormap, dst) {
  ...
  if (Array.isArray(colormap)) {
    colormap = Color.interpolate(colormap);
  } else if (typeof colormap === 'object') {
    colormap = Color.interpolate(parseColorObject(colormap.hue));
...
function parseColorObject(hues) {
  ...
  var cm = Color.hueArc(
    hues.start, hues.end, hues.saturation/100, hues.brightness/100);
  return Color.freeze(cm, hues.colorStops);
}
